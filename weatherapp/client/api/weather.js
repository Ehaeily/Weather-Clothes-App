...
const forecastEndpoint = params => {
    // Check if latitude and longitude are provided, else use city name
    const locationQuery = params.lat && params.lon
        ? `${params.lat},${params.lon}`
        : params.cityName;
    
    return `https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${locationQuery}&days=${params.days}&aqi=no&alerts=no`;
};
